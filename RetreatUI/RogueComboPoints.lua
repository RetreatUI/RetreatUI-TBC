...
local PACKAGE = RUI.weakAuraPackages and RUI.weakAuraPackages.rogue
if not PACKAGE or type(PACKAGE.Build) ~= "function" then return end

local ROOT = "RetreatUI TBC — Rogue Combo Points"
...
local function RogueLoad()
  return {
    class = { multi = { ROGUE = true }, single = "ROGUE" },
    spec = { multi = {} },
    use_class = true,
    use_never = false,
  }
  ...
  return {
    [1] = {
      trigger = {
        ...
        custom_hide = "custom",
        custom = code,
        events = "PLAYER_ENTERING_WORLD PLAYER_TARGET_CHANGED PLAYER_COMBO_POINTS UNIT_POWER_UPDATE UNIT_DISPLAYPOWER",
        unit = "player",
        debuffType = "HELPFUL",
        names = {},
        spellIds = {},
