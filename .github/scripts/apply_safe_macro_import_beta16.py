...
workflow = Path(".github/workflows/publish-beta.yml")
text = workflow.read_text(encoding="utf-8")
old = "          assert 'pcall(EditMacro, index, definition.name, icon, definition.body, 1)' in text\n"
new = "          assert 'pcall(EditMacro, index, definition.name, icon, definition.body, 1)' not in text\n          assert 'return true, \\\"preserved\\\"' in text\n          assert 'existing macros preserved' in text\n"
assert text.count(old) == 1
text = text.replace(old, new)

start = text.index("          NOTES=$(cat <<'EOF'\n")
end = text.index("          EOF\n          )", start) + len("          EOF\n          )")
notes = """          NOTES=$(cat <<'EOF'
          RetreatUI TBC beta 16.

          Includes:
          - Makes the class macro installer safe to run repeatedly.
          - Existing Character Specific `RUI ...` macros are preserved exactly and are never overwritten by a reinstall.
          - Only missing class macros are created.
          - User-edited Hunter timing values, equipment placeholders, targeting conditions and other custom macro changes survive future installer runs.
          - General Macro slots remain untouched.
          - The installer button now clearly says `INSTALL MISSING MACROS`.
          - Existing WeakAuras, ElvUI, Plater, Details, DBM and HUD behavior is unchanged.

          This remains a testing prerelease.
          EOF
          )"""
